"""
Backtest for the XGBoost ensemble.
Uses ATR stops, a 50MA filter, conviction-drop exits, and earnings avoidance.
...
# trading parameters (same as the transformer backtest)
MIN_CONF = 0.55   # lowered vs transformer (0.75) since XGB probs are more conservative
...
POSITION_SIZE = 20_000   # flat $20k per position (simplified from tiered)
...
# Analyst features - fetched once at the start of the run.
def fetch_analyst_features(ticker):
    try:
        ...
        if recs is not None and not recs.empty:
            ...
            features["analyst_buy_pct"] = (sb + b) / total
            features["analyst_sell_pct"] = (s + ss) / total
            ...
            if len(recs) >= 3 and "period" in recs.columns:
                ...
                if not old.empty:
                    osb = float(old.get("strongBuy", [0]).iloc[0]) if "strongBuy" in old else 0
                    ob = float(old.get("buy", [0]).iloc[0]) if "buy" in old else 0
                    oh = float(old.get("hold", [0]).iloc[0]) if "hold" in old else 0
                    os_ = float(old.get("sell", [0]).iloc[0]) if "sell" in old else 0
                    oss = float(old.get("strongSell", [0]).iloc[0]) if "strongSell" in old else 0
                    ot = osb + ob + oh + os_ + oss + 1e-8
                    features["analyst_upgrade_momentum"] = features["analyst_buy_pct"] - (osb + ob) / ot
                else:
                    features["analyst_upgrade_momentum"] = 0.0
            else:
                features["analyst_upgrade_momentum"] = 0.0
        else:
            for k in ["analyst_buy_pct", "analyst_sell_pct", "analyst_hold_pct",
                      "analyst_score", "analyst_upgrade_momentum"]:
                features[k] = np.nan
        ...
        if targets is not None and hasattr(info, "last_price"):
            cp = info.last_price
            if cp and cp > 0:
                mt = targets.get("mean", None)
                features["analyst_upside"] = (mt - cp) / cp if mt else np.nan
            else:
        ...
        if revisions is not None and not revisions.empty:
            ...
            features["eps_revision_30d"] = (up30 - down30) / (up30 + down30 + 1e-8)
            features["eps_revision_7d"] = (up7 - down7) / (up7 + down7 + 1e-8)
        else:
...
# global price + feature caches, filled by preload_all_data()
PRICE_CACHE = {}
FEATURE_CACHE = {}


def download_prices(ticker, backtest_date, days=300):
    """Download enough history for all indicators to warm up (200MA needs ~200 bars)."""
    end = datetime.strptime(backtest_date, "%Y-%m-%d")
...
def preload_all_data():
    ...
    global PRICE_CACHE, FEATURE_CACHE
    ...
    for ticker in TICKERS:
        try:
            ...
            FEATURE_CACHE[ticker] = feat_df

        except Exception as e:
            print(f"    FAILED: {e}")

    print(f"\nPreloaded {len(FEATURE_CACHE)} tickers")


# Feature engineering - identical to data.py compute_features().
...
def add_macro_features(df, macro_df):
    """Join pre-computed macro features onto ticker df by date."""
    return df.join(macro_df, how="left", rsuffix="_macro")


def compute_macro_df(spy_df, vix_df):
    ...
    if vix_df is not None and not vix_df.empty:
        vix = vix_df["close"].reindex(macro.index, method="ffill")
        macro["vix_level"] = vix
        macro["vix_vs_ma20"] = vix / (vix.rolling(20).mean() + 1e-8) - 1
    else:
...
def normalise_row(row_dict, stats):
    """Normalise a single feature dict using per-ticker train stats."""
    out = {}
    for k, v in row_dict.items():
        if k in stats:
            out[k] = (v - stats[k]["mean"]) / (stats[k]["std"] + 1e-8)
        else:
            out[k] = v
    return out
...
def predict_prob(xgb_model, lr_model, imputer, feature_vector, top_feat_idx):
    """Return ensemble probability for a single feature vector."""
    x = feature_vector.reshape(1, -1)

    xgb_prob = xgb_model.predict_proba(x)[0][1]

    # LR on top features only
    x_lr = x[:, top_feat_idx]
    x_lr = imputer.transform(x_lr)
    lr_prob = lr_model.predict_proba(x_lr)[0][1]

    return ENSEMBLE_XGB_WEIGHT * xgb_prob + ENSEMBLE_LR_WEIGHT * lr_prob


def build_feature_vector(ticker, backtest_date, all_stats, analyst_cache,
                         macro_df, feature_cols, top_feat_idx,
                         xgb_model, lr_model, imputer):
    """Build features for ticker on a given day and return a prediction dict, or None on failure."""
    try:
        ...
        # only use data available up to today
        df = full_df.loc[:backtest_date].copy()

        if len(df) < 210:
            return None
        df = add_macro_features(df, macro_df)

        # analyst features - static broadcast
        analyst_feats = analyst_cache.get(ticker, {})
        for feat_name in ["analyst_buy_pct", "analyst_sell_pct", "analyst_hold_pct",
                          "analyst_score", "analyst_upgrade_momentum",
                          "analyst_upside", "eps_revision_30d", "eps_revision_7d"]:
            df[feat_name] = analyst_feats.get(feat_name, np.nan)

        # drop warmup rows
        df.dropna(subset=["rsi_14", "macd", "slope_20", "rsi_mean_14"], inplace=True)
        if df.empty:
            return None
        ...
        # build feature dict from latest row
        raw_row = {col: float(latest[col]) if col in latest.index else np.nan
                   for col in feature_cols}

        # normalise using per-ticker train stats
        ticker_stats = all_stats.get(ticker, all_stats.get("AAPL", {}))
        norm_row = normalise_row(raw_row, ticker_stats)

        feature_vector = np.array([norm_row.get(col, np.nan) for col in feature_cols],
                                  dtype=np.float32)

        if np.isnan(feature_vector).mean() > 0.3:   # >30% NaN, skip
            return None

        feature_vector = np.nan_to_num(feature_vector, nan=0.0)

        prob = predict_prob(xgb_model, lr_model, imputer, feature_vector, top_feat_idx)
        ...
        print(f"[{ticker} | {backtest_date}] FAILED: {e}")
        return None


def main():
    print("=" * 70)
    print("  XGBoost Batch Backtest")
    ...
    # features are already normalised so filling with 0 is fine
    imputer = SimpleImputer(strategy="constant", fill_value=0.0)
    imputer.fit(np.zeros((1, len(top_features))))

    print(f"  XGBoost loaded ({len(feature_cols)} features)")
    print(f"  LR loaded (top {len(top_features)} features)")

    # preload all price data + features
    preload_all_data()

    # download SPY + VIX for macro features
    print("\nDownloading SPY and VIX for macro features...")
    # longer window to cover the full backtest period
    spy_start = (datetime.strptime(START_DATE, "%Y-%m-%d") - timedelta(days=365)).strftime("%Y-%m-%d")

    spy_df = yf.download("SPY", start=spy_start, end=END_DATE,
    ...
    try:
        vix_df = yf.download("^VIX", start=spy_start, end=END_DATE,
                             interval="1d", auto_adjust=True, progress=False)
        ...
        vix_df = None
        print("  VIX unavailable - will use NaN")

    macro_df = compute_macro_df(spy_df, vix_df)
    print(f"  Macro features computed: {macro_df.shape[1]} columns")

    # fetch analyst features once at start
    print("\nFetching analyst features (once)...")
    analyst_cache = {}
    for ticker in TICKERS:
        ...
        time.sleep(0.25)
    print(f"  Fetched for {len(analyst_cache)}/{len(TICKERS)} tickers")
    ...
    for day in trading_days:
        ...
        # build features and predict for all tickers
        all_results = {}
        for ticker in TICKERS:
            result = build_feature_vector(
                ticker, today, all_stats, analyst_cache,
                macro_df, feature_cols, top_feat_idx,
                xgb_model, lr_model, imputer
            )
            if result:
        ...
        for trade in open_trades:
            ...
            if exit_reason:
                ...
                trade_log.append({
                    ...
                    "prob_at_entry": trade["prob"],
                    "prob_at_exit": round(current_prob, 4),
                ...
                print(f"  {today}  SELL  {ticker:<6}  {exit_reason:<20}  "
        ...
        for ticker, result in all_results.items():
            ...
            if ticker in cooldown_tickers:
                continue
            if not result["above_50ma"]:   # 50MA filter
                continue
            candidates[ticker] = result

        # rank by probability, take top N
        ranked = sorted(candidates.items(), key=lambda x: x[1]["prob"], reverse=True)

        for ticker, result in ranked[:slots_available]:
            prob = result["prob"]
    ...
    print(f"\n{'='*70}")
    print(f"  BACKTEST COMPLETE - XGBoost Ensemble")
    print(f"{'='*70}")
    ...
    for reason, grp in sells.groupby("exit_reason"):
        avg = grp["net_pnl"].mean()
        print(f"    {reason:<22} {len(grp):3d} trades  avg P&L ${avg:+.2f}")
